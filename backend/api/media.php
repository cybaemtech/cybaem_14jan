...
// For cPanel: Set UPLOAD_DIR to the absolute path, e.g., /home/username/public_html/uploads/
function getUploadDir() {
    $envDir = getenv('UPLOAD_DIR');
    if ($envDir && !empty(trim($envDir))) {
        $dir = rtrim(trim($envDir), '/') . '/';
        return $dir;
    }
    
    // Detect environment and set appropriate upload directory
    // For cPanel deployment
    if (isset($_SERVER['HTTP_HOST']) && !strpos($_SERVER['HTTP_HOST'], 'replit')) {
        // cPanel environment - uploads go to public_html/public/uploads/
        return dirname(dirname(__DIR__)) . '/public/uploads/';
    }
    
    // Default for Replit environment
    return __DIR__ . '/../../public/uploads/';
}

// Get base URL for uploaded files
function getUploadBaseUrl() {
    // Since images are actually served from /public/uploads/ on your live site,
    // we should generate URLs that match this structure
...
switch ($method) {
    ...
    case 'POST':
        ...
        if (isset($_FILES['file'])) {
            ...
            if (move_uploaded_file($file['tmp_name'], $file_path)) {
                $url = getUploadBaseUrl() . $file_name;
    ...
    case 'DELETE':
        ...
        if ($media && strpos($media['url'], '/uploads/') === 0) {
            $file_name = basename($media['url']);
            $file_path = getUploadDir() . $file_name;
